package com.zitemaker.amazingtitles.code.internal.loaders;
...
public class PluginLoader {
	...
	public static void loadDefaultAnimations() {
		...
		none.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 20, 20,
				DisplayType.TITLE));
		...
		symbolWrap.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE,
				20, 10, DisplayType.TITLE));
		...
		rainbow.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 20,
				20, DisplayType.TITLE));
		rainbow.setFramesBuilder((arguments, args) -> {
			...
			for (int i = 0; i <= length; i++) {
				if (ColorTranslator.isInsidePlaceholder(smoothed, i))
					continue;
				String in = smoothed.substring(0, i);
				String out = smoothed.substring(i);
				String var = "<" + red + ">&l" + in + "</" + blue + "><" + blue + ">&l" + out + "</" + green + ">";
				frames.add(ColorTranslator.extractVisibleRange(ColorTranslator.colorize(var), padding, inputLen));
			}
			for (int i = 0; i <= length; i++) {
				if (ColorTranslator.isInsidePlaceholder(smoothed, i))
					continue;
		...
		AnimationBuilder waves = new AnimationBuilder(null, AnimationType.REPEATING, true, "Color1(HEX/Legacy)",
				"Color2(HEX/Legacy)");
		waves.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 10, 20,
				DisplayType.TITLE));
		...
		AnimationBuilder bounce = new AnimationBuilder(null, AnimationType.REPEATING, true, "Color1(Hex/Legacy)",
				"Color2(Hex/Legacy)");
		bounce.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 10,
				20, DisplayType.TITLE));
		bounce.setFramesBuilder((arguments, args) -> {
			...
			for (int i = 0; i <= length; i++) {
				...
				if (to.length() == 1 || from.length() == 1)
					continue;
				String formatted = "<" + color1 + ">&l" + to + "</" + color2 + "><" + color2 + ">&l" + from + "</"
						+ color1 + ">";
				frames.add(ColorTranslator.extractVisibleRange(ColorTranslator.colorize(formatted), padding, inputLen));
			}
		...
		flashing.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 10,
				10, DisplayType.TITLE));
		...
		split.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 10, 10,
				DisplayType.TITLE));
		...
		words_split.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE,
				10, 10, DisplayType.TITLE));
		...
		from_left.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 20,
				20, DisplayType.TITLE));
		...
		from_right.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE,
				20, 20, DisplayType.TITLE));
		...
		fade_in.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 20,
				20, DisplayType.TITLE));
		fade_in.setFramesBuilder((arguments, args) -> {
			LinkedList<String> frames = new LinkedList<>();
			List<String> chars = ColorTranslator
					.charactersWithColors(ColorTranslator.colorize(arguments.getMainText()));
		...
		AnimationBuilder fade_in_writer = new AnimationBuilder(null, AnimationType.FADE_IN, false,
				"Writer(Single Word)");
		fade_in_writer.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null",
				BarColor.WHITE, 20, 20, DisplayType.TITLE));
		...
		from_both_sides.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null",
				BarColor.WHITE, 20, 20, DisplayType.TITLE));
		...
		AnimationBuilder pulsing = new AnimationBuilder(null, AnimationType.REPEATING, true, "Color1(Hex/Legacy)",
				"Color2(Hex/Legacy)");
		pulsing.setComponentArguments(ComponentArguments.create("Text is null", "SubText is null", BarColor.WHITE, 20,
				20, DisplayType.TITLE));
		pulsing.setFramesBuilder((arguments, args) -> {
			...
			int total_max = 0;
			if (r_total > total_max)
				total_max = r_total;
			if (g_total > total_max)
				total_max = g_total;

...
public class CHMessages implements CommandHandler {
	...
	public BaseComponent[] helpMessage() {
		...
		builder.appendLegacy(" &7> &7/at sendMessage <Players> <Message>\n", "&{#ffa6fc}Click to suggest command",
				ClickEvent.Action.SUGGEST_COMMAND, "/at sendMessage ");
		builder.appendLegacy("§f");
		return builder.createMessage();
	}
	...
	public boolean readAndExecute(CommandSender s, String[] args) {
		...
		String finalRaw = builder.length() > 0 ? builder.substring(1) : "";
		for (Player player : players) {
			String resolved = PlaceholderAPIIntegration.resolve(player, finalRaw);
			BaseComponent[] message = InteractiveMessageHelper.getMessageFromRaw(resolved);
